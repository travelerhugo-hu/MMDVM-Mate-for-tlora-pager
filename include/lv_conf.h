/**
 * MMDVM Mate project copy of the LilyGoLib LVGL configuration.
 * Deltas vs. upstream: LV_USE_PERF_MONITOR 0 (the FPS overlay sits exactly on
 * top of our bottom hint banner) and LV_MEM_SIZE bumped to 96 kB.
 * This file must stay in include/ so it shadows LilyGoLib/src/lv_conf.h for
 * *every* translation unit (PlatformIO puts the project include dir first).
 *
 * @file lv_conf.h
...
/* MMDVM Mate only ever renders five sizes. The upstream config enables all 21
 * Montserrat faces, which costs roughly half a megabyte of flash for glyphs
 * that are never drawn - so everything unused is switched off here.
 *   12 -> bottom hint banner        14 -> lists, settings rows (LV_FONT_DEFAULT)
 *   16 -> top banner                20 -> talkgroup line
 *   40 -> the big callsign / FREE                                            */
